...
class TestCritiqueTextEndpoint:
    ...
    def test_domain_biomedical(self, client):
        res = client.post(
            "/api/v1/critique/text", json={"report_text": _SHORT, "domain": "biomedical"}
        )
        assert res.status_code == 200
    ...
    def test_domain_cs(self, client):
        ...
        res = client.post("/api/v1/critique/text", json={"report_text": cs_text, "domain": "cs"})
        assert res.status_code == 200
        assert res.json()["omega_score"] is not None
    ...
    def test_domain_math(self, client):
        ...
        res = client.post(
            "/api/v1/critique/text", json={"report_text": math_text, "domain": "math"}
        )
        assert res.status_code == 200

    def test_unknown_domain_raises_422_or_400(self, client):
        res = client.post(
            "/api/v1/critique/text", json={"report_text": _SHORT, "domain": "physics_unknown_xyz"}
        )
    ...
    def test_round_number_reflected(self, client):
        res = client.post("/api/v1/critique/text", json={"report_text": _SHORT, "round_number": 2})
...
class TestReviewSimEndpoint:
    def test_basic_review_sim(self, client):
        res = client.post("/api/v1/review-sim", json={"report_text": _SHORT, "reviewers": 3})
        assert res.status_code == 200
        data = res.json()
    ...
    def test_two_reviewers(self, client):
        res = client.post("/api/v1/review-sim", json={"report_text": _SHORT, "reviewers": 2})
        assert res.status_code == 200
...
class TestRebuttalEndpoint:
    def test_basic_rebuttal(self, client):
        res = client.post("/api/v1/rebuttal", json={"report_text": _SHORT, "style": "ieee"})
        assert res.status_code == 200
        data = res.json()
    ...
    def test_rebuttal_styles(self, client):
        for style in ("ieee", "acm", "nature"):
            res = client.post("/api/v1/rebuttal", json={"report_text": _SHORT, "style": style})
            assert res.status_code == 200
    ...
    def test_invalid_style_returns_400(self, client):
        res = client.post("/api/v1/rebuttal", json={"report_text": _SHORT, "style": "lancet"})
        assert res.status_code == 400

    def test_rebuttal_item_fields(self, client):
        res = client.post("/api/v1/rebuttal", json={"report_text": _SHORT, "style": "acm"})
        data = res.json()
        if data["items"]:
            item = data["items"][0]
...
class TestResponseLetterEndpoint:
    def test_ieee_letter(self, client):
        res = client.post("/api/v1/response-letter", json={"report_text": _SHORT, "style": "ieee"})
        assert res.status_code == 200
        data = res.json()
    ...
    def test_acm_letter(self, client):
        res = client.post("/api/v1/response-letter", json={"report_text": _SHORT, "style": "acm"})
        assert res.status_code == 200
        assert "ACM" in res.json()["markdown"]

    def test_nature_letter(self, client):
        res = client.post(
            "/api/v1/response-letter", json={"report_text": _SHORT, "style": "nature"}
        )
        assert res.status_code == 200
...
class TestJournalScoreEndpoint:
    def test_default_journal(self, client):
        res = client.post(
            "/api/v1/journal-score", json={"report_text": _SHORT, "journal": "default"}
        )
        assert res.status_code == 200
        data = res.json()
    ...
    def test_all_journals(self, client):
        for journal in ("nature", "ieee", "lancet", "q1", "q2", "q3", "default"):
            res = client.post(
                "/api/v1/journal-score", json={"report_text": _SHORT, "journal": journal}
            )
            assert res.status_code == 200, f"Failed for journal={journal}"

    def test_calibrated_omega_in_range(self, client):
        res = client.post("/api/v1/journal-score", json={"report_text": _SHORT, "journal": "ieee"})
        data = res.json()
...
class TestDiffEndpoint:
    def test_basic_diff(self, client):
        res = client.post(
            "/api/v1/diff", json={"report_v1_text": _SHORT, "report_v2_text": _SHORT_V2}
        )
        assert res.status_code == 200
        data = res.json()
        assert "rcs" in data
    ...
    def test_identical_texts_rcs(self, client):
        res = client.post("/api/v1/diff", json={"report_v1_text": _SHORT, "report_v2_text": _SHORT})
        assert res.status_code == 200
