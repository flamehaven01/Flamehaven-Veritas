...
_EXP_PATTERNS: list[tuple[re.Pattern, ExperimentClass]] = [
    (re.compile(r'\b(reproduc|parity|identity.verif|baseline.repro)', re.I), ExperimentClass.PARITY),
    ...
    (re.compile(r'\b(multi.axis|cross.cycle|EXP-\d{3}.*EXP-\d{3})',   re.I), ExperimentClass.MULTIAXIS),
...
def step0_classify(text: str) -> tuple[ExperimentClass, ExperimentClass | None, str]:
    ...
    for pattern, cls in _EXP_PATTERNS:
        if pattern.search(text):
            matched.append(cls)
    if not matched:
        return ExperimentClass.EXTENSION, None, "No explicit class markers found; defaulting to EXTENSION."
    primary = matched[0]
...
_SCOPE_VIOLATION = re.compile(
    ...
    r'physical yield|real.world improvement|superior|outperforms)\b', re.I
)
...
def step1_claim_integrity(text: str) -> tuple[StepResult, list[HoldEvent]]:
    ...
    if viols:
        ...
        findings.append(StepFinding(
            "1.2", f"Scope boundary violation: {len(viols)} overreaching term(s) detected.",
            TraceabilityClass.NOT_TRACEABLE, verbatim_quote=quote,
        ))
    else:
        findings.append(StepFinding("1.2", "No scope boundary violation detected.", TraceabilityClass.TRACEABLE))

    # 1.3 Verdict legitimacy
    tc_verdict = TraceabilityClass.TRACEABLE if (verdicts and has_numbers_near_verdicts) else (
    ...
    for h in holds:
        ...
        findings.append(StepFinding("1.4", f"{h.event_id}: disposition={h.disposition.value}", tc_hold, hold_event=h))

    viol_sentence = f"Scope boundary overreach at '{viols[0]}' is the most vulnerable claim." if viols else \
        "Verdict legitimacy is the most vulnerable point when numeric traceability is absent."
    ...
    hold_part = f" {len(holds)} HOLD event(s) detected; {sum(1 for h in holds if not h.cause_stated)} lack stated cause." if holds else ""

    prose = (f"The central claim is {tc_claim.value} from the report body.{scope_part}"
             f"{verdict_part}{hold_part}")

    result = StepResult(
        step_id="1", weight=0.40, prose=prose,
        findings=findings, vulnerable_claim=viol_sentence,
    )
    return result, holds
...
def step2_traceability(text: str, holds: list[HoldEvent]) -> StepResult:
    ...
    findings.append(StepFinding(
        ...
        TraceabilityClass.TRACEABLE if figs_without_anchor == 0 else TraceabilityClass.PARTIALLY_TRACEABLE,
    ))
    findings.append(StepFinding(
        "2.2",
    ...
    findings.append(StepFinding(
        "2.3",
        "Audit trace fields present without stated interpretation." if figs_without_anchor > 0 else "No interpretation gap detected.",
        TraceabilityClass.PARTIALLY_TRACEABLE if figs_without_anchor > 0 else TraceabilityClass.TRACEABLE,
    ))
    findings.append(StepFinding(
        "2.4",
        f"Cross-cycle comparison spans {len(cycles)} cycle(s)." + (" Mixed baselines risk." if cross_cycle_mixed else ""),
        TraceabilityClass.PARTIALLY_TRACEABLE if cross_cycle_mixed else TraceabilityClass.TRACEABLE,
    ))
    ...
    prose = (
        ...
        + (f"Cross-cycle comparison mixes {len(cycles)} cycle anchors, which is invalid regardless of numeric stability." if cross_cycle_mixed else "")
    )
...
def step3_series_continuity(text: str) -> StepResult:
    ...
    if not has_prior:
        prose = ("No prior cycle reference exists; cross-cycle continuity is NOT APPLICABLE. "
    ...
    findings: list[StepFinding] = []
    findings.append(StepFinding("3.1", f"{len(open_qs)} open question(s) from prior cycle detected.",
        TraceabilityClass.TRACEABLE if open_qs else TraceabilityClass.PARTIALLY_TRACEABLE))
    findings.append(StepFinding("3.2", "Valid handoff scope stated." if handoff else "Handoff is a deferral, not a contract.",
        TraceabilityClass.TRACEABLE if handoff else TraceabilityClass.NOT_TRACEABLE))
    drift_quote = drift.group(0) if drift else None
    findings.append(StepFinding("3.3", "Narrative drift detected." if drift else "Governing frame holds.",
    ...
    prose = (
        ...
        + ("Handoff scope is explicitly stated." if handoff else "Handoff is a deferral — it names no opening question, out-of-scope items, or next-cycle constraints. ")
        + (f"Narrative drift detected at '{drift_quote}', reading stability as systemic superiority." if drift else "Governing frame is maintained.")
    )
...
def step4_publication_readiness(text: str) -> StepResult:
    ...
    findings: list[StepFinding] = []
    findings.append(StepFinding("4.1",
        "Internal rule violation: report prohibits hash-free figures but contains none." if self_contradict else "Internal rule compliance holds.",
        TraceabilityClass.NOT_TRACEABLE if self_contradict else TraceabilityClass.TRACEABLE))
    findings.append(StepFinding("4.2",
        f"Misread risk at '{misread.group(0)}' — external reader may infer causal superiority." if misread else "No high-risk misread point identified.",
    ...
    prose = (
        ("Self-contradiction detected: the report defines a no-hash-free-figures rule but cites figures without sha256. " if self_contradict else "Internal rule compliance holds. ")
        + (f"The highest misread risk is at '{misread.group(0)}', where an external reader is most likely to infer a causal claim the report does not intend." if misread else "No single high-risk misread point was identified.")
    )
...
def step5_priority_fix(
    ...
    ranked = sorted(
        ...
        key=lambda x: (-sum(1 for f in x[0].findings if f.traceability == TraceabilityClass.NOT_TRACEABLE), -x[1])
    )
    worst = ranked[0][0]
    second = ranked[1][0]
